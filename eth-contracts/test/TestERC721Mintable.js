  ...
  describe('match erc721 spec', function () {
    ...
    it('should return the  total supply', async function () {
      ...
      assert.equal(
        total.toNumber(),
        expectedTotal,
        `the Total tokens should be ${expectedTotal}`
      )
    })
    ...
    it('should get the token balance', async function () {
      ...
      assert.equal(
        balance.toNumber(),
        expectedBalance,
        `the account balance should be ${expectedBalance}`
      )
    })

    // token case: uri should be complete i.e: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/1
    it('should return token uri', async function () {
      ...
      assert(
        ...
        `the TokenURI of the token must be ${expectedTokenURI}`
      )
    })

    it('should transfer the token from one owner to another', async function () {
      const tokenId = 1
      ...
      assert.equal(
        currentOwner,
        account_four,
        `the account_four should be the new owner of ${tokenId}`
      )
    })
  })
  ...
  describe('have ownership properties', function () {
    ...
    it('should fail when minting when address is not contract owner', async function () {
      ...
      assert.equal(
        tokenMintedFailed,
        false,
        'the token minted when sending a message from account that does not own contract'
      )
    })

    it('should return contract owner', async function () {
      let owner = await this.contract.owner.call({ from: account_one })
      assert.equal(owner, account_one, 'the account_one should be contract owner')
    })
  })
})