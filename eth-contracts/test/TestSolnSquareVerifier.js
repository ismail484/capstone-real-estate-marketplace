...
contract('TestSolnSquareVerifier', accounts => {
  ...
  // Test case if a new solution can be added for contract - SolnSquareVerifier
  ...
  // Test case  if an ERC721 token can be minted for contract - SolnSquareVerifier
  ...
  it('An ERC721 token can be minted with incorrect proof', async function () {
    ...
    assert.equal(
      tokenMinted,
      false,
      'Token must not be minted with incorrect proof'
    )
  })
})
